...
class NN2(object):
    ...
    def backpropagation(self,
        ...
        cost = cross_entropy_cost(outs[-1], Y) # calculate cross entropy with softmax output
        # if cost > 100:
        #     print(cost)
...
# W, V = NN.train(X, Y, 150, epochs=2, batch_size=1, verbose=True)
...
print(round(metrics.accuracy_score(Y.flatten(), np.round(Yhat.flatten())), 3))

print("Predicting test set...")
X_test, Y_test = preprocess(test_images, test_labels)
Yhat = NN.predict(X_test, W, V)
print(round(metrics.accuracy_score(Y_test.flatten(), np.round(Yhat.flatten())), 3))
